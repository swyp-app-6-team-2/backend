...
- **기존 가입자 재로그인:** 기존 `/social-login`이 `requiresTermsAgreement=false`와 로그인 토큰을 반환한다. 약관 화면은 생략한다. 온보딩은 추가된 `onboardingRequired`로 별도 판단한다. 기존 회원은 V15에서 완료로 간주하며, 신규 가입 후 온보딩 미완료 계정은 재로그인 시에도 온보딩을 진행한다. 자세한 연동은 `ONBOARDING.md`를 참고한다. 기존 회원에게 새 ageOver14Agreed 동의를 강제로 요구하지 않는다.
