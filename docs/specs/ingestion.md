...
- **수집**: 공개 embed(`/p/{code}/embed/captioned/`, Reel은 `/reel/{code}/…`)의 `contextJSON`에서 caption과 카드 순서대로의 미디어를 읽는다. embed가 영상 주소를 주지 않은 Reel만 유료 수집 API로 한 번 더 찾는다(`embed가 막힌 Reel은 유료 수집으로 보완`). 로그인 수집과 앱 쪽 수집은 쓰지 않는다.
...
- **보조 수집(Apify)**: 공개 embed로 영상 주소를 얻지 못했을 때, 유료 외부 수집 API는 **Reel에만** 한 번 더 부르고, 캡션과 대표 이미지로 하는 분석은 영상 주소를 얻지 못한 단일 영상 게시물에도 적용한다. 외부 수집은 재시도하지 않고 첫 시도에서만 부른다.
...
### 4.10. Instagram은 공개 embed 우선, 막힌 Reel만 유료 수집
...
| **유료 수집 API를 보조로 (2026-09-18 추가 채택)** | embed가 영상을 주지 않는 Reel과 서버 IP 차단을 함께 덮는다 | 건당 비용과 외부 의존이 생기고, 사용자가 공유한 링크가 제3자에게 넘어간다 |

- **2026-09-14 결정.** 공식 경로로는 기능 자체를 만들 수 없어 리스크를 받아들였다. 막혔을 때의 대체 수집 경로는 두지 않았다.
- **2026-09-18 변경.** 음원(라이선스 음악)을 쓴 Reel은 embed가 영상 주소를 아예 주지 않아 100% 실패한다는 것이 실측으로 드러났다(Reel 9개 비교에서 `uses_original_audio` 하나가 성공·실패를 갈랐고, 서버 IP가 속도 제한에 걸린 사례도 나왔다). 그래서 **embed가 영상을 주지 못한 Reel에 한해 유료 수집 API(Apify)를 보조로 부른다.** 첫 시도에만, 재시도 없이, 남은 시간이 충분할 때만 부르고, 토큰이 비면 이 경로는 꺼진다. 그래도 영상을 못 얻으면 캡션과 대표 이미지로 분석한다.
- **보조 경로에서 외부로 나가는 것**은 사용자가 공유한 Instagram 링크(정규화한 주소) 하나다. 캡션·분석 결과·사용자 식별 정보는 보내지 않는다. 받은 영상 주소는 기존 허용 호스트 규칙을 통과한 것만 내려받고, 응답이 요청한 게시물의 것인지 코드로 대조한다.
