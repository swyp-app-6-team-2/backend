...
| **같은 프로세스 + 설정으로 켜고 끔 (채택)** | 배포 단위 하나, 인프라 추가 없음   | 분석이 API와 메모리·커넥션 풀을 나눠 씀. 동시 처리 수로 제한                      |

- **설정으로 켠다.** Worker는 `ingestion.worker.enabled`로 켜고, 기본값은 꺼짐이다. 운영 배포에서만 켠다.
  - 팀원 로컬은 Gemini API 키 없이 뜨고, 만든 Job은 처리되지 않은 채 10분 뒤 실패로 정리된다(§3.4).
  - Worker를 켰는데 키가 없으면 기동을 실패시킨다.
- **분리는 코드 변경 없이 할 수 있다.** 필요해지면 같은 이미지를 Worker 전용으로 하나 더 띄우고 API 쪽 설정을 끈다.
