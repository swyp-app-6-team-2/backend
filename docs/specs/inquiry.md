...
Account ── 탈퇴 시 문의 일괄 삭제 요청 ──▶ Inquiry
```

Account 도메인의 코드는 바꾸지 않는다. 관리자 조회 쿼리가 `users`·`profiles` 테이블을 읽기만 한다.

**탈퇴 시 정리.** Account가 `InquiryCleanupService.deleteAllForUser`를 호출하면 해당 사용자의 문의와 첨부를 지운다. Account가 사용자 행을 잠근 트랜잭션 안에서만 실행한다. 전체 흐름과 도메인 간 순서는 [User Withdrawal Spec](./user-withdraw.md)의 `처리 흐름`이 소유한다.
...
API는 코드만 주고받고 표시 이름은 앱이 보유한다. 서버의 표시 이름(`InquiryType`)은 관리자 페이지와 운영 Discord 알림에서만 쓴다.
...
| 인증 | 폼 로그인. `POST /admin/login`, 성공하면 원래 요청한 주소로 돌아간다. 저장된 요청이 없으면 `/admin/inquiries` |
| 로그아웃 | `POST /admin/logout`, 로그인 화면으로 이동 |
| 세션 | 서버 메모리 세션, 요청 없이 8시간이면 만료. 만료된 세션으로 요청하면 `/admin/login?expired`로 보내되 **원래 요청한 주소를 기억해** 로그인 뒤 그리로 돌아간다 |
...
- 세션 만료를 8시간으로 둔 이유는 Discord 알림 링크다. 30분이면 링크를 누를 때마다 로그인 화면을 거친다. remember-me 쿠키는 두지 않는다 — 쿠키 탈취 시 장기간 접근을 허용하는 대가에 비해, 앱을 재시작하면 어차피 세션이 사라지는 지금 구조에서 얻는 것이 적다.
...
| 작업 | 경계 | 동시성 |
|---|---|---|
| 문의 접수 | `@Transactional` 하나: 요청 순서대로 Key마다 Upload 연결 → `inquiry` 저장. 커밋 이후 Discord 알림 1회(트랜잭션 밖) | Key 연결은 Upload의 조건부 UPDATE라, 같은 Key를 동시에 연결해도 한 요청만 성공한다 |
| 내 목록 | 읽기 트랜잭션 | — |
...
문의 접수 알림의 웹훅 주소는 `application.yml`·`docker-compose.vm.yml` app `environment`·`.env.vm.example` 세 곳에 둔다. 비어 있어도 앱은 뜨고 알림만 보내지 않으므로 테스트 설정에는 두지 않는다.

```yaml
inquiry:
  # 비어 있으면 접수 알림을 보내지 않는다.
...
새 문의가 접수되면 운영 Discord 채널에 한 줄을 보낸다. 사용자에게 가는 알림이 아니다.

- 보내는 것: 문의 번호, 유형(표시 이름), 제목, 접수 시각(KST), 관리자 상세 링크. **내용·사진은 보내지 않는다.** 내용에는 연락처 같은 개인정보가 섞이기 쉽다.
- 예: `문의 #4 · 별 슬롯 확장 · 광고 봤는데 슬롯이 안 늘어나요 · 2026-09-17 18:47 · <https://dev-api.starpick.cloud/admin/inquiries/4>`
- 제목은 사용자 입력이라 줄바꿈을 공백으로 바꾸고, 멘션(`@everyone` 등)과 링크 미리보기를 막아 보낸다.
- 접수 트랜잭션이 커밋된 뒤 한 번만 시도하고 재시도하지 않는다. 실패해도 접수는 `201` 이다.
- 답변 저장은 알리지 않는다. 웹훅 주소가 없는 환경에서는 보내지 않는다.
...
| Discord 알림이 거절됨(4xx·5xx) | WARN | `inquiryId`, 상태 코드 |
| Discord 알림 발송 실패(타임아웃·링크 생성 실패) | WARN | `inquiryId`, 예외 클래스명 |
...
- Discord 웹훅 주소는 남기지 않는다. 예외 메시지에 주소가 들어갈 수 있어 메시지를 그대로 찍지 않는다.
...
| Discord 장애·타임아웃, 또는 웹훅 주소 미설정 | 그 알림은 누락된다. 접수는 `201` 이고 문의는 관리자 목록에 남는다 |
| Discord 응답이 느림 | 접수 응답이 그만큼(최대 5초) 늦어진다. 발송이 커밋 뒤 같은 요청 스레드에서 돌기 때문이다 |
| 배포로 앱이 재시작됨 | 세션이 사라져 관리자가 다시 로그인해야 한다. 로그인하면 원래 보려던 문의로 돌아간다 |
...
~~탈퇴 방식과 Inquiry 정리~~ **확정 완료(2026-09-16)** — 하드 삭제로 정해졌고 `§2.3 도메인 협력`에 반영했다.
