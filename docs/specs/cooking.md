...
| 핵심 기술 결정은 무엇인가?      | Recipe 존재·소유권을 사전 검증하고, `photoKey` 중복은 UNIQUE로 추가 방어. `recipe_id` FK는 아직 없음(잠금 전략과 함께 정할 후속 과제)  |
...
| 속성         | 필수 | 규칙                                      |
|------------|----|-----------------------------------------|
| `recipeId` | O  | 조리한 Recipe. FK 없는 스칼라이며 이유는 `트랜잭션과 동시성 제어`에 있다 |
...
**`cook_history.recipe_id`에는 FK가 없다.** 2026-09-07에는 Cooking이 Recipe Entity를 참조할 수 없어 두지 않았다. 지금은 스키마를 Flyway가 소유해 Java 매핑과 무관하게 FK를 선언할 수 있지만, Recipe 삭제의 잠금·삭제 순서와 함께 정할 후속 과제로 남겨 두었다.
...
두 번째 행이 FK를 두지 않아 생긴 빈틈이다. 사용자에게는 드러나지 않는다. 목록은 `recipeId`로 조회하고 그 Recipe는 이미 `404`이기 때문이다. 발생하려면 같은 사용자가 두 지점에서 동시에 조작해야 한다. FK를 추가하면 이 행은 첫 번째 행으로 흡수되고, FK 위반을 `404 + RECIPE_NOT_FOUND`로 변환해야 한다.
...
| 사전 검증 + FK                   | DB가 잘못된 관계 저장을 차단              | (당시) Cooking이 Recipe Entity를 참조해야 해 경계가 깨짐 |
| **사전 검증만 (채택)**              | 생성 흐름이 단순하고 도메인 경계를 그대로 유지     | 검증과 저장 사이의 경쟁에서 고아 데이터를 허용            |

CookHistory 생성에서는 Recipe 존재와 소유권을 확인하되 Recipe 행을 잠그지 않는다.

FK를 최종 방어선으로 쓰는 안은 당시 도메인 경계 규칙과 충돌해 채택하지 못했다. Flyway 도입으로 이 충돌은 사라졌지만 FK 추가는 아직 정하지 않았다. 그 전까지 허용하는 빈틈은 `트랜잭션과 동시성 제어`에 적었다.
