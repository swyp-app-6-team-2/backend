...
| 핵심 기술 결정은 무엇인가?      | Recipe 존재·소유권을 사전 검증하고, `photoKey` 중복은 UNIQUE로 추가 방어. `recipe_id` FK가 삭제와의 경쟁을 막음  |
...
| 속성         | 필수 | 규칙                                      |
|------------|----|-----------------------------------------|
| `recipeId` | O  | 조리한 Recipe. `recipe.id`를 참조하는 FK가 있으며 동작은 `트랜잭션과 동시성 제어`에 있다 |
...
**`cook_history.recipe_id`는 `recipe.id`를 참조한다(`fk_cook_history_recipe`).** 2026-09-07에는 Cooking이 Recipe Entity를 참조할 수 없어 두지 않았고, 스키마를 Flyway가 소유하게 된 뒤 Java 매핑과 무관하게 선언했다. `on delete cascade`는 쓰지 않는다. DB가 조리 기록을 지우면 `photo_key`로 저장소를 정리할 기회를 잃기 때문이다. 삭제 순서는 애플리케이션이 지키고(자식 먼저 → 부모) FK는 그 순서를 어기지 못하게만 한다.

Recipe 삭제와 CookHistory 생성이 경쟁하면 결과는 다음과 같다. Recipe 삭제는 대상 행을 `FOR UPDATE`로 잡고, FK가 있는 insert는 같은 행에 `FOR KEY SHARE`를 요구한다. 두 잠금이 충돌하므로 순서가 이 지점에서 정해진다.

| 실행 순서                     | 결과                                                        |
|---------------------------|-----------------------------------------------------------|
| Recipe 삭제가 소유권 검증보다 먼저 완료 | CookHistory 생성 실패 → `404 + RECIPE_NOT_FOUND`              |
| Recipe 삭제가 소유권 검증과 저장 사이  | 저장이 삭제 커밋까지 대기했다가 FK 위반 → `404 + RECIPE_NOT_FOUND`       |
| CookHistory 생성이 먼저 완료     | 생성 성공 후 Recipe 삭제 시 해당 CookHistory도 함께 삭제 가능               |

두 번째 행이 예전에는 고아 데이터를 남기던 빈틈이었고, FK가 첫 번째 행으로 흡수했다. 삭제 쪽이 실패하는 경우는 없다 — 생성이 먼저 잠금을 잡으면 삭제가 기다렸다가 자식·부모를 순서대로 지운다(2026-09-18 실측). 생성 요청이 받는 FK 위반은 제약 이름으로 판별해 `404 + RECIPE_NOT_FOUND`로 변환한다.

#### 이력 조회

Recipe 존재·소유권 확인과 CookHistory 목록 조회는 각각 짧은 읽기 트랜잭션에서 끝내고, **조회 URL 서명은 트랜잭션 밖에서 수행한다.**

[Image Upload Common Spec](./upload.md)의 `트랜잭션과 원격 호출 경계`가 기록한 대로 서명에는 저장소 클라이언트 타임아웃이 걸리지 않는다. 이력 조회는 페이지네이션이 없어 N건을 순차로 서명하므로, 서명을 읽기 트랜잭션 안에 두면 요청 하나가 DB 커넥션을 그만큼 오래 점유한다. CookHistory에는 지연 로딩 컬렉션이 없어 트랜잭션 밖에서 응답을 조립해도 안전하다.
...
| **사전 검증 + FK (채택)**          | DB가 잘못된 관계 저장을 차단, 생성 흐름은 그대로 | 스키마가 도메인 경계 밖의 관계를 알게 됨               |
| 사전 검증만                        | 스키마도 도메인 경계를 그대로 유지            | 검증과 저장 사이의 경쟁에서 고아 데이터를 허용            |

CookHistory 생성에서는 Recipe 존재와 소유권을 확인하되 Recipe 행을 잠그지 않는다. 잠금 대신 FK를 최종 방어선으로 쓴다.

FK 안은 처음에는 Cooking이 Recipe Entity를 참조해야 해 채택하지 못했다. Flyway가 스키마를 소유하면서 Java 매핑과 무관하게 FK를 선언할 수 있게 되어 그 충돌이 사라졌고, 2026-09-18에 추가했다. 잠금 설계는 따로 필요하지 않다 — 근거는 `트랜잭션과 동시성 제어`에 적었다.
